...
this.title_container=$("<h4 />",{"class":"ui-pnotify-title"}).appendTo(this.container);if(!this.options.title){this.title_container.hide();}else if(this.options.title_escape){this.title_container.text(this.options.title);}else{this.title_container.html(this.options.title);}
...
this.options.stack.animation=false;this.runModules('init');if(this.options.auto_display){this.open();}
...
if(that.elem.is(":visible")){if(this){this.call();}
...
if(that.elem.css("opacity")=="0"||!that.elem.is(":visible")){that.elem.removeClass("ui-pnotify-in");if(this){this.call();}
...
posTimer=null;if(PNotify.notices&&PNotify.notices.length){$.each(PNotify.notices,function(){var s=this.options.stack;if(!s){return;}
